...
# from FlagEmbedding import FlagReranker  # 新增導入 FlagReranker
...
#             score_inputs = [[query, doc] for doc in top_k_docs]
...
# # Function to normalize scores to a specified range
# # def normalize_scores(scores, new_min=-1, new_max=1):
# #     """
# #     將分數列表標準化到指定範圍內。

# #     Args:
# #         scores (list of float): 原始分數列表。
# #         new_min (float): 新的最小值。
# #         new_max (float): 新的最大值。

# #     Returns:
# #         list of float: 標準化後的分數列表。
# #     """
...
# #     normalized = [new_min + (score - min_score) / (max_score - min_score) * (new_max - new_min) for score in scores]
# #     return normalized
...
#         # 平均分數
#         if len(normalized_reranker1_scores) != len(top_k_docs) or len(reranker2_scores) != len(top_k_docs):
#             logging.error(f"Score length mismatch for QID: {qid}")
#             answer_dict['answers'].append({"qid": qid, "retrieve": None})
#             continue
